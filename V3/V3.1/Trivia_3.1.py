###new###
#delete where is says "kss_on"
#changes on kv file = change design on welcomescreen label and add on_leave func
###new-end###
...
class WelcomeScreen(Screen):
	...
	def entry(self):
		...
		###new###
		if self.manager.initial_welcome:
			self.ids.welcome_label.text = algorithm.get_display("ברוך הבא! אנא המתן לשאלות.")
		else:
			self.ids.welcome_label.text = algorithm.get_display(" תודה ששיחקת! מקווים שהמשחק ריענן אותך.\n\n הניקוד שלך: "+ str(self.manager.score) + ".\n\n בקרוב נמשיך לשחק.")

	def leave(self):
		self.manager.initial_welcome = False
	###new-end###
...
kv = Builder.load_file('Trivia_3.1.kv')
