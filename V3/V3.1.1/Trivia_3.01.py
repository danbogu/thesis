...
congif_file_path = r"C:\TriviaData\trivia_config.json"
...
kss_result_file_path = str(config['Kss_result_file_path'])
...
class ScoreScreen(Screen):
	def entry(self):
		...
		Clock.schedule_once(self.to_welcome_screen,5)
...
class KSS(Screen):
	...
	def click(self, ans_num):
		...
		if ans_num > kss_threshold:
			...
			#while True:
			#	try:
			#		kss_result_file = open(kss_result_file_path, "r")
			#		kss_result_file.write("1")
			#		kss_result_file.close()
			#		break
			#	except:
			#		continue
			degraded_fitness_popup.open()
...
kv = Builder.load_file('Trivia_3.01.kv')
